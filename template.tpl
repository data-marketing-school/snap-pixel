...
const level = copyFromDataLayer('level');
const search_term = copyFromDataLayer('search_term');
const method = copyFromDataLayer('method');
const shipping_tier = copyFromDataLayer('shipping_tier');
...
function getEventParameters() {
  ...
  if(data.sendEcommerceData) {
    event_parameters.transaction_id = ecommerce.transaction_id;
    event_parameters.price = ecommerce.value;
    event_parameters.currency = ecommerce.currency;
    ...
    event_parameters.level = level;
    event_parameters.search_string = search_term;
    event_parameters.sign_up_method = method;
    event_parameters.delivery_method = shipping_tier;
    //add more according to GA4 standard
  }
...
function getUserParameters() {
  ...
  if(data.sendUserData) {
    if(user_data.email_address) {
      user_data.user_email = user_data.email_address;
    }
    
    if(user_data.sha256_email_address) {
      user_data.user_hashed_email = user_data.sha256_email_address;
    }
  
    if(user_data.phone_number) {
      user_data.user_phone_number = user_data.phone_number;
    }
    
    if(user_data.sha256_phone_number) {
      user_data.user_hashed_phone_number = user_data.sha256_phone_number;
    }
    
    if(user_data.address) {
      
      if(user_data.address.first_name) {
        user_data.firstname = user_data.address.first_name;
      }
      
      if(user_data.address.last_name) {
        user_data.lastname = user_data.address.last_name;
      }
      
      if(user_data.address.city) {
        user_data.geo_city = user_data.address.city;
      }
      
      if(user_data.address.country) {
        user_data.geo_country = user_data.address.country;
      }
      
      if(user_data.address.postal_code) {
        user_data.geo_postal_code = user_data.address.postal_code;
      }
      
      if(user_data.address.region) {
        user_data.geo_region = user_data.address.region;
      }
...
function loadSnapScript() {
  ...
  snaptr('track', data.pixelId, data.eventName, merge(getUserParameters(), getEventParameters()));
...
Created on 22/10/2024, 18:31:59
